...
export default function MyAdsPage() {
  ...
  const filtered: Tyre[] =
    currentTab === 'favorites'
      ...
      : myTyres.filter((tyre) => {
          ...
          switch (currentTab) {
            case 'active':
              return !tyre.isDeleted && !isExpiredNow && willBeDeletedAtDate >= now;
            case 'expired':
              return isExpiredNow;
            case 'deleted':
  ...
  return (
    <>
      <Helmet>
        <title>Мої оголошення | Omega Auto</title>
        <meta
          name="description"
          content="Керуйте своїми оголошеннями про продаж шин, редагуйте та видаляйте їх."
        />
      </Helmet>
        ...
        <nav
          ...
          {TABS.map((tab) => (
            <button
              key={tab}
              type="button"
              className={`flex-shrink-0 py-2 px-4 sm:py-3 sm:px-6 border-b-4 transition font-semibold text-sm sm:text-base
              ${
                currentTab === tab
